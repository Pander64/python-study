...
def read_urls(filename):
  ...
  increasing order."""
  # +++your code here+++
  # Extract the hosname if the filename
  underbar = filename.index('_')
  host = filename[underbar + 1:]

  # Store the urls into a dict to screen out the duplicate 
  url_dict = {}

  f = open(filename)
  for line in f:
    # Find the path which is the after GET and surrounder by space.
    match = re.search(r'"GET (\S+)', line)
    # Above uses \S (upper case S) which is any not-space char 
    # Alternative could use square brackets: "Get ([^ ])"
    # or the ? form: "GET (.+?) "
