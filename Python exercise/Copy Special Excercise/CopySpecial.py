...
def main():
  ...
  def get_special_path(dirname):
    ...
    result = []
    paths = os.listdir(dirname) # list of path in that dir
    for fname in paths:
  ...
  def copy_to(paths, to_dir):
    ...
    for path in paths:
      ...
      # Could error out if alredy exists os.path.exists():

  def zip_to(paths, zipfile):
    """Zip up all of the given file into a new zip file with the given name."""
    cmd = 'zip -j ' + zipfile + ' ' + ' '.join(path)
    print("Command I'm going to do:" + cmd)
    (status, output) = subprocess.getstatusoutput(cmd) 
    # If command had a problem (status inn non-zero)
